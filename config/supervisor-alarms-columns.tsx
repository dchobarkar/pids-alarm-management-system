...
export interface SupervisorAlarmsTableActions {
  onAssignClick: (alarm: AlarmWithRelations) => void;
  onEscalate: (alarmId: string) => void;
}

/** Table columns for supervisor alarms list. Assign and Escalate actions. */
export const getSupervisorAlarmsColumns = (
  actions: SupervisorAlarmsTableActions,
  ...
  },
  {
    ...
    key: "actions",
    render: (r) => {
      const canEscalate = ["UNASSIGNED", "ASSIGNED", "IN_PROGRESS"].includes(
        r.status,
      );
      const showAssign =
        r.status === "UNASSIGNED";
      const showEscalate = canEscalate;
      if (!showAssign && !showEscalate) return "—";
      return (
        <div className="flex flex-wrap items-center gap-2">
          {showAssign && (
            <Button
              ...
              onClick={() => actions.onAssignClick(r)}
            >
              Assign
            </Button>
          )}
          {showEscalate && (
            <Button
              ...
              variant="ghost"
              className="text-sm text-(--brand-primary) hover:underline"
              onClick={() => actions.onEscalate(r.id)}
            >
              Escalate
            </Button>
          )}
